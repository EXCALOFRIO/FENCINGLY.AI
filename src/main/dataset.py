        ...
        with ThreadPoolExecutor() as executor:
            for archivo_json in tqdm(os.listdir(directory), desc="Loading data"):
                ...
                with open(ruta_json) as f:
                    ...
                    sequence = np.zeros((num_frames, 2, 50))  # Initialize matrix for the sequence without confidence
                    for i, frame in enumerate(datos_json['frames'][:num_frames]):
                        ...
                        for j, pose in enumerate(poses):
                            ...
                            keypoints_without_confidence = []
                            for k in range(0, len(keypoints), 3):
                                keypoints_without_confidence.extend(keypoints[k:k+2])  # Take only x and y
                            sequence[i, j, :] = keypoints_without_confidence
...
def dividir_datos_entrenamiento_validacion(dataset, porcentaje_entrenamiento):
    datos = np.array(dataset.data)
    datos_entrenamiento, datos_validacion = train_test_split(datos, test_size=1 - porcentaje_entrenamiento, random_state=42)
    return datos_entrenamiento, datos_validacion
    
def preparar_datos(datos_entrenamiento_izquierda, datos_entrenamiento_derecha, datos_validacion_izquierda, datos_validacion_derecha):
    ...
    return datos_entrenamiento, datos_validacion, etiquetas_entrenamiento, etiquetas_validacion
    
def convertir_a_tensores(datos_entrenamiento, datos_validacion, etiquetas_entrenamiento, etiquetas_validacion):
...
def cargar_y_preparar_datos(izquierda_dir, derecha_dir, porcentaje_entrenamiento):
    # Cargar datos
    dataset_izquierda, dataset_derecha = cargar_datos(izquierda_dir, derecha_dir)
    
    # Dividir datos
    datos_entrenamiento_izquierda, datos_validacion_izquierda = dividir_datos_entrenamiento_validacion(dataset_izquierda, porcentaje_entrenamiento)
    datos_entrenamiento_derecha, datos_validacion_derecha = dividir_datos_entrenamiento_validacion(dataset_derecha, porcentaje_entrenamiento)
    
    # Preparar datos
    datos_entrenamiento, datos_validacion, etiquetas_entrenamiento, etiquetas_validacion = preparar_datos(datos_entrenamiento_izquierda, datos_entrenamiento_derecha, datos_validacion_izquierda, datos_validacion_derecha)
    ...
    datos_entrenamiento, datos_validacion, etiquetas_entrenamiento, etiquetas_validacion = convertir_a_tensores(datos_entrenamiento, datos_validacion, etiquetas_entrenamiento, etiquetas_validacion)
